...
export default function AnalysisDetailView({ jobId, onBack }) {
  ...
  const handleExportMarkdown = () => {
    ...
    md += `| Attribute | Value |\n`;
    md += `| :--- | :--- |\n`;
    md += `| **Scan Target** | \`${job?.sourceIdentifier || 'Target Repository'}\` |\n`;
    md += `| **Job ID** | \`${jobId}\` |\n`;
...
<div class="container">
  ...
  <div class="card">
    ...
    <p>Security: ${job?.metrics?.securityScore || 100}/100 &bull; Quality: ${job?.metrics?.qualityScore || 100}/100 &bull; Maintainability: ${job?.metrics?.maintainabilityScore || 100}/100 &bull; Architectural: ${job?.metrics?.architecturalScore || 100}/100</p>
  </div>
  ${findings.map(f => `
    <div class="card">
    ...
    <div className="max-w-7xl mx-auto px-2 sm:px-6 py-4 sm:py-8 space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        ...
        {!isScanning && job?.status === 'COMPLETED' && (
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5 w-full sm:w-auto">
            {/* Direct Quick Share Sheet Opener */}
            <button
              onClick={() => handleOpenExport('pdf')}
              className="px-3 sm:px-3.5 py-1.5 bg-blue-600 hover:bg-blue-500 text-white text-[11px] sm:text-xs font-bold rounded-xl sm:rounded-full flex items-center justify-center space-x-1.5 transition-all shadow-sm shadow-blue-500/20 active:scale-95 cursor-pointer font-display"
            >
              <Share2 className="w-3.5 h-3.5" />
              <span>Share Report</span>
            </button>

            {/* 4 Format Direct Selectors */}
            <div className="grid grid-cols-4 gap-1.5 sm:flex sm:items-center sm:space-x-1.5">
              ...
              <button
                ...
                title="Download or View HTML report"
                className="px-2.5 sm:px-3 py-1.5 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 text-[11px] sm:text-xs font-semibold rounded-xl sm:rounded-full flex items-center justify-center space-x-1 sm:space-x-1.5 transition-colors border border-slate-200 dark:border-slate-800 shadow-sm cursor-pointer active:scale-95"
              >
      ...
      {!isScanning && job?.status === 'COMPLETED' && (
        <>
          ...
          {/* Mobile View Switcher (Findings vs File Tree) */}
      ...
      {/* Native Export & Share Modal for Mobile and Desktop */}
      <ExportShareModal
